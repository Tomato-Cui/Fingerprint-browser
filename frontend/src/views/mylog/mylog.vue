<template>
  <div class="flex flex-col p-4 bg-gray-50 h-main">
      ...
      <div class="flex overflow-hidden flex-col flex-1 bg-white rounded-lg shadow" style="padding: 20px"> <!-- Set fixed height here -->
        <!-- Filters -->
        <div class="flex gap-4 mb-6">
          <div class="relative">
            <Select>
              ...
              <SelectContent>
                <SelectGroup>
                  <SelectItem value="所有成员">
                    所有成员
                  </SelectItem>
                  <SelectItem value="选择2">
                    选择2
                  </SelectItem>
                  <SelectItem value="选择3">
                    选择3
                  </SelectItem>
                </SelectGroup>
              </SelectContent>
      ...
      <div class="overflow-auto flex-1">
        <table class="w-full">
          ...
          <tr>
            ...
            <!--                </th>-->
            <th class="p-4 text-center text-sm font-medium text-gray-600">成员</th>
            <th class="p-4 text-center text-sm font-medium text-gray-600">操作</th>
            <th class="p-4 text-center text-sm font-medium text-gray-600">具体内容</th>
            <th class="p-4 text-center text-sm font-medium text-gray-600">时间</th>
            <th class="p-4 text-center text-sm font-medium text-gray-600">结果</th>
            <th class="p-4 text-center text-sm font-medium text-gray-600">操作者IP</th>
          </tr>
          </thead>
          ...
          >
            ...
            <td class="p-4 text-center text-sm">{{ item.member }}</td>
            <td class="p-4 text-center text-sm">{{ item.operation }}</td>
            <td class="p-4 text-center text-sm">{{ item.content }}</td>
            <td class="p-4 text-center text-sm">{{ item.time }}</td>
            <td class="p-4 text-center text-sm">
              <div>
                    <span
                        class="px-2 py-1 rounded text-xs"
                        :class="item.result === '成功' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'"
                    >
                      {{ item.result }}
                    </span>
              </div>
            ...
            <td class="p-4 text-center text-sm">
              <div class="flex items-center justify-center gap-2">
                <img src="" alt="US Flag" class="w-5 h-5" />
                <span>{{ item.ip }}</span>
                ...
                <Popover v-model:open="item.logisopen">
                  ...
                  <PopoverContent class="min-w-24 w-24 p-2 text-sm space-y-2" align="center">
                    <p>{{ item.ip }}</p>
                    <p>{{ item.operation }}</p>
                  </PopoverContent>
                </Popover>
      ...
      <div class="bg-white  p-4 mt-auto flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="p-2 hover:bg-gray-100 rounded" :disabled="currentPage === totalPages">
...
for (let i = 0; i < 130; i++) {
  const newAgentsData = {
    ...
    member: "成员" + i,
    operation: "修改窗口",
    content: "修改了2个窗口，序号11",
    time: "2024-01-11 14:30:00",
    result: "失败",
    ip: "MEIGO.CN",
    logisopen: false,
  };
