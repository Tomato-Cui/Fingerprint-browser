...
const options = ref([...new Array(20).keys().map(item => (`新建环境${item + 1}`))]);
const checkeds = ref([]);
const search = ref('');

const appendHandle = (item) => {
    if (!checkeds.value.includes(item)) {
        checkeds.value.push(item)
    }
}
const removeHandle = (item) => {
    checkeds.value = checkeds.value.filter(i => i != item)
}
const checkAllHandle = () => {
    checkeds.value = options.value;
}
...
watch(search, (newV) => {
    if (newV) {
        ...
        options.value = [...new Array(20).keys().map(item => (`新建环境${item + 1}`))];
    }
});
</script>

<template>
    <Model class="max-w-[1125px]" :title="props.title" description="(请选择需要添加该扩展的环境)" :open="props.open"
        @close="() => emit('close')">
        <!-- 主体 -->
            ...
            <div class="w-full rounded overflow-hidden">
                <div class="flex w-full gap-x-4">
                    <div class="flex-1 flex items-center text-sm h-12 gap-x-4">
                        <p :class="cn('cursor-pointer', tabIndex == 0 && 'border-b-2 border-blue-500 text-blue-500')"
                            @click="() => tabIndex = 0">按环境</p>
                        <p :class="cn('cursor-pointer', tabIndex == 1 && 'border-b-2 border-blue-500 text-blue-500')"
                            @click="() => tabIndex = 1">按平台</p>
                    </div>
                ...
                <div class="flex gap-x-4 h-[28rem]">
                    ...
                    <div v-if="tabIndex == 0" class="flex-1 border rounded-md flex flex-col p-2">
                        <div class="">
                            <div class="relative">
                                <SearchIcon class="absolute left-2 top-1.5 z-10 text-gray-500" />
                                <Input class="w-full text-sm text-gray-600 pl-10" v-model="search" />
                            </div>
                            <div class="px-4 flex gap-x-2 text-sm py-2 select-none">
                                <input type="checkbox" id="check-all" @change="(v) =>
                        ...
                        <div class="overflow-auto flex-auto flex flex-col gap-y-4 py-1">
                            <button v-for="item in options"
                                @click="() => checkeds.includes(item) ? removeHandle(item) : appendHandle(item)"
                                :class="cn('text-left px-4 hover:text-primary flex justify-between', checkeds.includes(item) ? 'text-primary' : '')">
                                {{ item }}
                                <CheckIcon v-if="checkeds.includes(item)" class="w-3 h-3" />
                            </button>
                        </div>
                    </div>
                    <div class="border rounded-md flex-1 p-2">
                        <div class="overflow-auto flex flex-wrap gap-2">
                            <button v-for="item in checkeds"
                                ...
                                {{ item }}
                                <XIcon @click="() => removeHandle(item)" class="w-4 h-4 cursor-pointer" />
                            </button>
                        </div>
                    </div>
                </div>
