<template>
    <!-- <div v-show="props.open" class="fixed inset-0 z-50 flex items-start justify-end"> -->
        ...
        <div v-show="props.open" class="mt-16 mr-4 min-w-72 rounded-lg bg-white p-4 shadow-lg z-50">
...
const allActions = ref([
    ...
    { key: 'env', label: '修改环境信息', icon: HomeSmileIcon, visible: true },
    { key: 'transfer', label: '转移', icon: RoundTransferHorizontal, visible: true },
    { key: 'cancelTransfer', label: '取消转移', icon: ForbidRoundTransferHorizontal, visible: true },
    { key: 'clearCache', label: '清除缓存', icon: ClearCacheIcon, visible: true },
    { key: 'delEnvironment', label: '删除环境', icon: TrashBinTrashIcon, visible: true },
])
