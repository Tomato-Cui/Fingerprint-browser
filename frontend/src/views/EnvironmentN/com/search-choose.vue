<template>
  <Transition name="modal-fade">
    <div
      v-show="props.open"
      ref="targetDiv"
      class="z-30 p-4 w-full max-w-md bg-white rounded-lg border shadow-sm"
    >
      ...
      <div class="space-y-3">
        ...
        <div v-for="(section, index) in sections" :key="index">
          <button
            ...
            class="flex justify-between items-center p-2 w-full rounded-lg border border-gray-100 hover:bg-gray-50"
          >
            <div class="flex items-center">
              {{ section.title }}
              <div
                v-if="section.hasNotification"
                class="ml-2 w-1.5 h-1.5 bg-red-500 rounded-full"
              ></div>
            </div>
            <AltArrowDownIcon
          ...
          <!-- Section Content -->
          <div v-show="section.isOpen" class="px-4 mt-2">
            <div
              v-if="section.type === 'timeRange'"
              class="flex items-center space-x-4"
            >
              <div class="flex-1">
                <input
                  type="datetime-local"
                  class="p-2 w-full text-sm rounded-lg border border-gray-200"
