<template>
  <Transition name="modal-fade">
    ...
    >
      <div class="flex justify-between items-center pb-1 mb-4 border-b">
        <h3 class="text-lg font-medium">快捷设置</h3>
        <TooltipButton title="重置">
          <RefreshSquareIcon class="cursor-pointer size-6" @click="refresh" />
        </TooltipButton>
      </div>
      <div class="pr-2 space-y-2" ref="divHeight">
        <div class="flex gap-4 justify-between items-center">
          <FrameIcon class="text-gray-300 size-5" />
          <div
            class="w-full h-[40px] bg-[#7744ff] flex rounded-lg items-center"
          >
        ...
        <TransitionGroup name="list" tag="div">
          <div
            ...
            :key="action.key"
            class="flex gap-4 justify-between items-center space-y-2 transition-all duration-300 ease-in-out"
          ...
          >
            <FrameIcon class="text-gray-700 cursor-move size-5" />
            <div
              class="flex items-center bg-[#EDEDFF80] w-full h-[40px] pl-[10px] rounded-lg"
            >
...
const allActions = ref([
  ...
  },
  {
    ...
    action: void 0,
    children: [
      { key: "addTab", label: "新增标签", active: () => emit("addLabel") },
      { key: "reset", label: "重设标签", active: () => emit("resetLabel") },
      { key: "clean", label: "清空标签", active: () => emit("cleanLabel") },
    ],
  },
  {
    key: "group",
    ...
    action: void 0,
    children: [
      { key: "addGroup", label: "增加分组", active: () => emit("addGroup") },
    ],
  },
  {
    key: "export",
  ...
  // , children: [{ key: 'export1', label: '导出已选', active: () => emit('exportEnv') }, { key: 'export2', label: '导出最近50条', active: () => emit('exportEnv') }]
  {
    ...
    icon: RulerPenIcon,
    visible: true,
    action: () => emit("editStartPage"),
  },
  {
    key: "ua",
    label: "修改UA",
    icon: RuleCrossPenIcon,
    visible: true,
    action: () => emit("editUa"),
  },
  {
  ...
  },
  {
    ...
    visible: true,
    action: () => emit("transferEnv"),
  },
  {
    key: "cancelTransfer",
    label: "取消转移",
    icon: ForbidRoundTransferHorizontal,
    visible: true,
    action: () => emit("untransferEnv"),
  },
  {
    key: "clearCache",
    label: "清除缓存",
...
watch(
  ...
  (_) => {
    ...
    allActions.value = allActions.value.map((item: any) => {
      //找到分组的操作
      if (item.key === "group") {
        //处理groupData的字段
...
onMounted(() => {
  environment_group_query(1, 1000).then((res: any) => {
    allActions.value.find((item: any) => {
      //找到分组的操作
      if (item.key === "group") {
