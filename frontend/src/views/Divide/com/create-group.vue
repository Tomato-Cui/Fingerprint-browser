...
import {
  environment_group_create,
  environment_group_query,
} from "@/commands/environment-group";
...
<template>
  ...
  >
    <!-- 主体 -->
    <div class="flex flex-col gap-y-4 px-2 mb-6">
      <div class="flex gap-4 items-center">
        ...
        <div class="w-full">
          <Input
            ...
            class="flex-1 w-full"
          />
          <p v-show="errorRepeat" class="text-sm text-red-500">分组名称重复</p>
        </div>
      </div>
      <div class="flex gap-4 items-center">
    ...
    <div class="flex justify-end border-t">
      <div class="flex gap-x-4 justify-start py-4 mr-2">
        <PrimaryButton class="px-8" @click="subMit">确定 </PrimaryButton>
        <CancelButton
          class="px-8"
          @click="() => emit('update:createGroupDialog', false)"
          >取消
        </CancelButton>
      </div>
    </div>
