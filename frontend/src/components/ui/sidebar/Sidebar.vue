...
<template>
  <div v-if="collapsible === 'none'"
    :class="cn('flex h-full w-[--sidebar-width] min-w-[--sidebar-width] flex-col bg-sidebar text-sidebar-foreground', props.class)"
    v-bind="$attrs">
    <slot />
  </div>
  ...
  <div v-else class="group peer block" :data-state="state" :data-collapsible="state === 'collapsed' ? collapsible : ''"
