...
mod start_browser {
    ...
    pub struct Param {
        pub browser_id: i8, // 环境ID，创建环境成功后生成的唯一ID。
    ...
    pub async fn handle(Json(payload): Json<Param>) -> impl IntoResponse {
        browser::start(payload.browser_id)
            .await
...
mod stop_browser {
    ...
    /// 用于关闭对应的浏览器，需要指定环境ID。
    pub async fn handle(Json(payload): Json<Param>) -> impl IntoResponse {
        match browser::stop(payload.browser_id).await {
            Ok(response) => {
                let exit_status = response.data;
                let code = exit_status.unwrap_or_default().code();
                AppResponse::success(None, code)
            }
            Err(e) => AppResponse::fail(Some(e.to_string())),
        }
    }
}
...
mod view_active_browser {
    ...
    ///
    /// 查询当前设备所有已打开的浏览器。
    pub async fn handle(Json(_payload): Json<ViewActiveBrowserRequestParam>) -> impl IntoResponse {
