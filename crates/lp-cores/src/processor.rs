...
impl BrowserChildInfo {
    ...
    pub async fn format(&self) -> Result<Vec<String>, anyhow::Error> {
        ...
        let app_config = lp_states::config::get_config().unwrap();
        let current_dir = std::env::current_dir().unwrap();
        let app_data = lp_states::config::APP_DATA.clone();
        ...
        if self.environemnt_info.proxy_enable == 1 {
            ...
            args.push(if !host.is_empty() {
                ...
                format!(
                    "--proxy-server=socks5://{}",
                    lp_commons::util::get_proxy_from_registry().unwrap_or_default()
                )
            });
        }
