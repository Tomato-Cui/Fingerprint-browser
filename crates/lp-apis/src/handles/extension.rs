...
pub async fn create(Json(payload): Json<lp_models::extension::Extension>) -> impl IntoResponse {
    match lp_services::extension::insert(payload).await {
...
pub async fn user_create(
    ...
    match lp_services::extension::user_insert(&state.user_uuid, &payload.extension_uuid).await {
        Ok(ok) => AppResponse::success(success_message("创建成功"), Some(ok)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn team_create(Json(payload): Json<TeamCreatePayload>) -> impl IntoResponse {
    match lp_services::extension::team_insert(&payload.team_id, &payload.extension_uuid).await {
        Ok(ok) => AppResponse::success(success_message("创建成功"), Some(ok)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn query_by_team(Json(payload): Json<QueryByTeamPayload>) -> impl IntoResponse {
    match lp_services::extension::query_by_team_id(
        payload.team_id,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn query_by_user(
    ...
    match lp_services::extension::query_by_user_uuid(
        &state.user_uuid,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn query_by_environment(
    ...
    match lp_services::extension::query_by_environmnet_uuid(
...
pub async fn query(Json(payload): Json<Pagination>) -> impl IntoResponse {
    match lp_services::extension::query(payload.page_num, payload.page_size).await {
        Ok(ok) => AppResponse::success(success_message("查询成功"), Some(ok)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn environment_use_extension(
    ...
    match lp_services::extension::environmnet_use_extension(
...
pub async fn environment_remove_extension(
    ...
    match lp_services::extension::environmnet_remove_extension(
        &payload.extension_uuid,
        &payload.environment_uuid,
    )
    .await
...
pub async fn update(Json(payload): Json<UpdatePayload>) -> impl IntoResponse {
    match lp_services::extension::update(&payload.extension_uuid, payload.extension).await {
        Ok(ok) => AppResponse::success(success_message("更新成功"), Some(ok)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn user_toggle_extension(
    ...
    match lp_services::extension::user_toggle_extension(
        &state.user_uuid,
        &payload.extension_uuid,
        payload.open,
    )
...
pub async fn delete_by_uuid(Json(payload): Json<ExtensionUUidPayload>) -> impl IntoResponse {
    match lp_services::extension::delete(&payload.extension_uuid).await {
...
pub async fn remove_by_user_uuid(
    ...
    match lp_services::extension::remove_by_user_uuid(&state.user_uuid, &payload.extension_uuid).await
    {
        Ok(ok) => AppResponse::success(success_message("移除成功"), Some(ok)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
