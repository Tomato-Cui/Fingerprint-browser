...
pub async fn start(Json(payload): Json<StartPayload>) -> impl IntoResponse {
    match lp_services::command::start_browser(&payload.environment_uuid).await {
...
pub async fn stops(Json(payload): Json<StopsPayload>) -> impl IntoResponse {
    match lp_services::command::stop(payload.environment_uuids).await {
        Ok(data) => {
...
pub async fn stop_all() -> impl IntoResponse {
    match lp_services::command::stop_all().await {
...
pub async fn status() -> impl IntoResponse {
    match lp_services::command::view_active().await {
        Ok(data) => {
