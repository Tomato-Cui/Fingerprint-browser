...
pub async fn query_by_id(Json(payload): Json<IdPayload>) -> impl IntoResponse {
    match lp_services::team::query_by_id(payload.id).await {
...
pub async fn is_leader(
    ...
    match lp_services::team::is_leader(&state.user_uuid, payload.id).await {
        Ok(data) => AppResponse::success(success_message("查询成功"), Some(data)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn query_search_by_name(
    ...
    match lp_services::team::query_by_search_name(&payload.team_name).await {
        Ok(data) => AppResponse::success(success_message("查询成功"), Some(data)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn query(
    ...
    match lp_services::team::query(&state.user_uuid, payload.page_num, payload.page_size).await {
        Ok(data) => AppResponse::success(success_message("查询成功"), Some(data)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn query_current_team(state: Extension<CurrentUser>) -> impl IntoResponse {
    match lp_services::team::query_current_team_info(&state.user_uuid).await {
        Ok(data) => AppResponse::success(success_message("查询成功"), Some(data)),
        Err(e) => AppResponse::fail(warn_message(e)),
    }
}
...
pub async fn query_team_all_user(
    ...
    match lp_services::team::query_team_all_user(
        &state.user_uuid,
        payload.team_id,
        payload.page_num,
        payload.page_size,
...
pub async fn query_team_all_blocked_user(
    ...
    match lp_services::team::query_team_all_blocked_user(
        &state.user_uuid,
        payload.team_id,
        payload.page_num,
        payload.page_size,
...
pub async fn query_team_group_all_user(
    ...
    match lp_services::team::query_team_group_all_user(
        &state.user_uuid,
        payload.team_id,
...
pub async fn blocked(
    ...
    match lp_services::team::blocked(
        &state.user_uuid,
...
pub async fn unblocked(
    ...
    match lp_services::team::un_blocked(
        &state.user_uuid,
        &payload.current_user_uuid,
        payload.team_id,
    )
...
pub async fn create(
    ...
    let team = lp_models::team::Team {
        name: payload.name.clone(),
        description: Some(payload.description.clone()),
        ..Default::default()
    };

    match lp_services::team::create(&state.user_uuid, &team).await {
...
pub async fn modify(payload: Json<ModifyTeamPayload>) -> impl IntoResponse {
    ...
    match lp_services::team::modify(id, &team).await {
...
pub async fn switch_team(
    ...
    match lp_services::team::switch_team(&state.user_uuid, payload.id).await {
...
pub async fn remove_current_user(
    ...
    match lp_services::team::remove_user(
        &state.user_uuid,
        payload.team_id,
        &payload.current_user_uuid,
    )
...
pub async fn modify_team_user_info(
    ...
    match lp_services::team::update_team_user_info(
        &state.user_uuid,
        payload.team_id,
...
pub async fn delete(Json(payload): Json<IdPayload>) -> impl IntoResponse {
    match lp_services::team::delete(payload.id).await {
