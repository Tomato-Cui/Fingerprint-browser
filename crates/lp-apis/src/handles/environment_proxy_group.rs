...
pub async fn query_by_id(Json(payload): Json<IdPayload>) -> impl IntoResponse {
    match lp_services::environment_proxy_group::query_by_group_id(payload.id).await {
...
pub async fn query(
    ...
    match lp_services::environment_proxy_group::query(
        &state.user_uuid,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn create(
    ...
    match lp_services::environment_proxy_group::create(&state.user_uuid, payload).await {
...
pub async fn modify(Json(payload): Json<ProxyGroup>) -> impl IntoResponse {
    ...
    match lp_services::environment_proxy_group::update(payload.id.unwrap_or_default() as u32, payload)
        .await
    {
...
pub async fn delete(Json(payload): Json<IdPayload>) -> impl IntoResponse {
    match lp_services::environment_proxy_group::delete(payload.id).await {
