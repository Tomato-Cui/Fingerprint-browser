...
use axum::Json;
use lp_models::environmnet_cookie::EnvironmentCookie;
...
pub async fn query_by_uuid(
    ...
    match lp_services::environmnet_cookie::query(&payload.environment_uuid).await {
        Ok(data) => {
...
pub async fn create(Json(payload): Json<CreatePayload>) -> impl IntoResponse {
    ...
    match lp_services::environmnet_cookie::create(&cookie).await {
...
pub async fn modify(Json(payload): Json<ModifyPayload>) -> impl IntoResponse {
    ...
    match lp_services::environmnet_cookie::modify(&payload.environment_uuid, &cookie).await {
...
pub async fn delete(Json(payload): Json<ModifyPayload>) -> impl IntoResponse {
    match lp_services::environmnet_cookie::delete(&payload.environment_uuid).await {
