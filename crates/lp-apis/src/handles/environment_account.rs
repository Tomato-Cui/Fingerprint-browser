...
pub async fn query_by_id(Json(payload): Json<QueryByIdPayload>) -> impl IntoResponse {
    match lp_services::environment_account::query_by_id(payload.id).await {
        Ok(data) => {
...
pub async fn query(
    ...
    match lp_services::environment_account::query(
        &state.user_uuid,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn query_current_by_current_environment(
    ...
    match lp_services::environment_account::account_query_current_environment(
...
pub async fn create(
    ...
    match lp_services::environment_account::create(&state.user_uuid, payload).await {
...
pub async fn modify(Json(payload): Json<EnvironmentAccount>) -> impl IntoResponse {
    ...
    match lp_services::environment_account::modify(&payload).await {
...
pub async fn delete(Json(payload): Json<DeleteByIdPayload>) -> impl IntoResponse {
    match lp_services::environment_account::delete(payload.id).await {
...
pub async fn batch_delete(Json(payload): Json<BatchDeletePayload>) -> impl IntoResponse {
    match lp_services::environment_account::batch_delete(payload.ids).await {
        Ok(data) => AppResponse::<bool>::success(success_message("删除成功"), Some(data)),
        Err(r) => AppResponse::<bool>::fail(warn_message(&format!("删除失败: {}", r))),
    }
}
