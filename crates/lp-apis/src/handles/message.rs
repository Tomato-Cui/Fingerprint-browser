...
pub async fn user_receive_query(
    ...
    match lp_services::user_team_temp::query_user_apply(
        &state.user_uuid,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn team_receive_query(Json(payload): Json<TeamReceiveQueryPayload>) -> impl IntoResponse {
    match lp_services::user_team_temp::query_team_apply(
        payload.team_id,
        payload.page_num,
        payload.page_size,
    )
...
pub async fn team_send(Json(payload): Json<TeamSendPayload>) -> impl IntoResponse {
    match lp_services::user_team_temp::team_send(
        &payload.user_uuid,
        payload.team_id,
        &payload.description,
    )
...
pub async fn user_send(
    ...
    match lp_services::user_team_temp::user_send(
        &state.user_uuid,
        &payload.team_name,
        &payload.description,
    )
...
pub async fn reject(Json(payload): Json<IdPayload>) -> impl IntoResponse {
    match lp_services::user_team_temp::delete(payload.id).await {
...
pub async fn team_allow(Json(payload): Json<TeamAllowPayload>) -> impl IntoResponse {
    match lp_services::user_team_temp::allow(payload.id, &payload.user_uuid, payload.team_id).await {
...
pub async fn user_allow(
    ...
    match lp_services::user_team_temp::allow(payload.id, &state.user_uuid, payload.team_id).await {
        Ok(data) => AppResponse::<bool>::success(success_message("通过成功"), Some(data)),
        Err(r) => AppResponse::<bool>::fail(warn_message(r)),
    }
}
