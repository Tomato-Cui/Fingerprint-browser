...
use lp_models::environment_proxies::Proxy;
...
pub async fn query_by_id(
    ...
    match lp_services::environment_proxy::query_by_id(&state.user_uuid, payload.id).await {
...
pub async fn query(
    ...
    match lp_services::environment_proxy::query(&state.user_uuid, payload.page_num, payload.page_size)
        .await
    {
...
pub async fn query_by_group(
    ...
    match lp_services::environment_proxy::query_by_group_id(
        &state.user_uuid,
...
pub async fn create(
    ...
    match lp_services::environment_proxy::create(&state.user_uuid, payload).await {
...
pub async fn modify(
    ...
    match lp_services::environment_proxy::update(
        &state.user_uuid,
...
pub async fn delete(
    ...
    match lp_services::environment_proxy::delete(&state.user_uuid, payload.id).await {
...
pub async fn batch_delete(
    ...
    match lp_services::environment_proxy::batch_delete(&state.user_uuid, payload.ids).await {
        Ok(data) => AppResponse::<bool>::success(success_message("删除成功"), Some(data)),
        Err(r) => AppResponse::<bool>::fail(warn_message(&format!("删除失败: {}", r))),
    }
}
