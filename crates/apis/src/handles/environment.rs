...
pub async fn query_by_uuid(
    ...
    match services::environment::query_environment_details(
        &state.user_uuid,
...
pub async fn modify_proxy(
    ...
    match services::environment_proxy::update_proxy(
        &state.user_uuid,
        &payload.environment_uuid,
        payload.porxy,
    )
...
pub async fn move_to_group(payload: Json<MoveToGroupPayload>) -> impl IntoResponse {
    match services::environment::move_to_group(&payload.environment_uuid, payload.group_id).await {
        Ok(ok) => AppResponse::<bool>::success(success_message("更新成功"), Some(ok)),
        Err(r) => AppResponse::<bool>::fail(warn_message(&format!("更新失败: {}", r))),
    }
}
...
pub async fn delete(
    ...
    match services::environment::delete(&state.user_uuid, &payload.environment_uuid).await {
...
pub async fn batch_delete(
    ...
    match services::environment::batch_delete(&state.user_uuid, payload.environment_uuids.clone())
        .await
    {
