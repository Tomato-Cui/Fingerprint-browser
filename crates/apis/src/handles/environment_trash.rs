...
pub async fn query_by_uuid(Json(payload): Json<EnvironmentUUIdPayload>) -> impl IntoResponse {
    match services::environment_trash::query_by_environment_uuid(&payload.environment_uuid).await {
        Ok(data) => AppResponse::<models::environment::Environment>::success(
            success_message("查询成功"),
            Some(data),
        ),
        Err(r) => AppResponse::<models::environment::Environment>::fail(warn_message(&format!(
            "查询失败: {}",
            r
        ))),
    }
...
pub async fn recover(
    ...
    match services::environment_trash::recover(&state.user_uuid, &payload.environment_uuid).await {
