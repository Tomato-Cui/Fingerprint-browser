...
pub async fn register(
    ...
    Ok(lp_services_remote::requests::user::regsiter(email, code, account, password).await?)
}
...
pub async fn user_query_search_by_email(email: &str) -> Result<JsonRespnse, tauri::Error> {
    Ok(lp_services_remote::requests::user::query_search_by_email(email).await?)
}
...
pub async fn reset_password(
    ...
    Ok(lp_services_remote::requests::user::reset_password(email, code, password).await?)
}
...
pub async fn register_send(email: &str) -> Result<JsonRespnse, tauri::Error> {
    Ok(lp_services_remote::requests::user::register_send(email).await?)
}

#[tauri::command]
pub async fn reset_password_send(email: &str) -> Result<JsonRespnse, tauri::Error> {
    Ok(lp_services_remote::requests::user::reset_password_send(email).await?)
}
